...
- 
**OAuth2 장점**
1. **편리한 사용자 경험**: 사용자는 별도의 회원가입 없이 기존의 Google, Facebook, Kakao 등의 계정으로 간편하게 로그인할 수 있습니다. 이는 로그인 과정을 단순화하고 사용자 경험을 향상시킵니다.
2. **보안 강화**: OAuth2 로그인은 비밀번호를 외부 인증 제공자에 맡기므로, 비밀번호 관리의 부담을 줄이고, 신뢰할 수 있는 보안 시스템을 이용하여 보안을 강화할 수 있습니다.
3. **개발 비용 절감**: 인증 시스템을 직접 구축하고 관리할 필요 없이 외부 인증 제공자를 활용함으로써 개발 및 유지 보수 비용을 절감할 수 있습니다.
...
4. **`principalOauth2UserService`에서 엑세스 토큰을 사용하여 Google 사용자 데이터 가져오기**  
   - 받은 엑세스 토큰을 이용해 애플리케이션은 Google의 사용자 정보를 OAuth2가 자동으로 가져온다.. ```java  OAuth2User oAuth2User = delegate.loadUser(userRequest);```
